...
# gets the parent directory path from bash
    parent_directory = os.environ["EXPORT"]
...
# Check if the parent directory exists
    ...
    destination_parent_directory_647 = parent_directory + "_Cav_aligned" 
    destination_parent_directory_568 = parent_directory + "_PTRF_aligned" 
...
# Iterate over subfolders
    ...
    for folder in glob.glob(os.path.join(cc, "*")):
        ...
        new_directory_cav = os.path.join(destination_parent_directory_647, folder_name)
        os.makedirs(new_directory_cav, exist_ok=True)

        # Create a folder for the PTRF files to go
        new_directory_ptrf = os.path.join(destination_parent_directory_568, folder_name)
        ...
        for file in glob.glob(os.path.join(folder, "*")):
            ...
            if ("aligned_c2" in file):
                shutil.move(file, os.path.join(new_directory_ptrf, get_name(file)))
